...
const scrapeSabayData = async () => {
  try {
    const { data } = await axios.get(
      "https://news.sabay.com.kh/topics/technology"
    );
    ...
    $(".row.list-item.item").each((index, element) => {
      ...
      if (!existingTitlesSet.has(title)) {
        scrapedItems.push({
          title,
          category: "បច្ចេកវិទ្យា", // Default category is technology
          description,
          articleUrl: fullUrl,
          photo,
        });
...
const startScrapeDataFromSabayTechnology1 = async () => {
...
module.exports = startScrapeDataFromSabayTechnology1;
